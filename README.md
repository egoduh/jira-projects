...
## Скрыть проекты (не относящиеся к разработке)

Создай рядом файл **`projects-exclude.txt`** (шаблон — `projects-exclude.txt.example`) и
перечисли ключи проектов, которые не показывать — по одному в строке:

```
# комментарии со #
HR
FIN
ADMIN*      # префикс со *: скроет ADMIN, ADMINOPS и т.п.
```

Исключённые проекты пропадают и из списка, и из дашборда (селектор + эталон).
Правки подхватываются при обновлении страницы — перезапуск `java` не нужен.
Файл в git не попадает (правишь под себя, не конфликтует при `git pull`).
